...
if ( !class_exists( '\\PACKS\\SHIPMENTS\\Admin\\AdminNotice' ) ) :
    class AdminNotice {
        ...
        const DISMISS_ACTION_PREFIX = 'packs_dismiss-';
        const DISMISSED_OPTION_PREFIX = 'packs_is_dismissed-';
        const DELAYED_NOTICE_OPTION = 'packs_delayed_notices';
        protected $id = null;
        protected $content = '';
        ...
        public function outputNotice() {
            ...
            if ($this->isPersistentlyDismissible) {
                $attributes['data-packs-dismiss-nonce'] = wp_create_nonce($this->getDismissActionName());
                $attributes['data-packs-notice-data'] = $this->toJson();
                $attributes['data-packs-signature'] = wp_create_nonce(
                    $this->id . '|' . $attributes['data-packs-dismiss-nonce'] . '|' . $attributes['data-packs-notice-data']
                );
        ...
        protected function enqueueScriptOnce() {
            if (!wp_script_is('packs-dismiss-notice', 'registered')) {
                //Note: Queueing a script also registers it.
                wp_enqueue_script(
                    'packs-dismiss-notice',
                    plugins_url('assets/js/dismiss-notice.js', __FILE__),
                    array('jquery'),
                    '20170318',
                    true
                );
